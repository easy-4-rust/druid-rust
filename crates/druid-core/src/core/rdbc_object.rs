...
//! `Array#getArray` 与 `Ref#getObject` 的返回对象。该类型属于 RDBC 平台层，
//! 不绑定某一种 Statement。

use super::{
    RdbcArray, RdbcBlob, RdbcClob, RdbcNClob, RdbcOpaqueObject, RdbcReader, RdbcRef, RdbcRowId,
    RdbcSqlXml, RdbcUrl, Value,
};
...
impl From<Value> for RdbcObject {
    fn from(value: Value) -> Self {
        Self::Scalar(value)
    }
}

impl fmt::Display for RdbcObject {
