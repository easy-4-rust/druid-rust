...
struct JdbcInputStreamState {
    reader: Option<Box<dyn Read + Send>>,
}

struct JdbcInputStreamInner {
    id: u64,
    state: Mutex<JdbcInputStreamState>,
}
...
/// `InputStream` 对象引用语义。
#[derive(Clone)]
pub struct JdbcInputStream {
    inner: Arc<JdbcInputStreamInner>,
}

impl JdbcInputStream {
    /// 包装一个真实输入流。
    ///
    /// # 参数
    ...
    pub fn new(reader: impl Read + Send + 'static) -> Self {
        Self {
            inner: Arc::new(JdbcInputStreamInner {
                id: NEXT_INPUT_STREAM_ID.fetch_add(1, Ordering::Relaxed),
                state: Mutex::new(JdbcInputStreamState {
...
impl fmt::Debug for JdbcInputStream {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("JdbcInputStream")
...
impl PartialEq for JdbcInputStream {
    fn eq(&self, other: &Self) -> bool {
        Arc::ptr_eq(&self.inner, &other.inner)
    }
}

impl Eq for JdbcInputStream {}

/// 二进制流 setter 的 Java 重载身份。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum JdbcStreamLength {
    /// 调用方使用未提供长度的重载。
    Unspecified,
