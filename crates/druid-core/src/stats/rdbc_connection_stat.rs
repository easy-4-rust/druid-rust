...
impl RdbcConnectionStatEntry {
    ...
    /// 返回当前管理快照。
    #[must_use]
    pub fn snapshot(&self) -> RdbcConnectionStatEntryValue {
        RdbcConnectionStatEntryValue {
            id: self.id,
...
/// `RdbcConnectionStat.Entry` 的不可变管理快照。
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "PascalCase")]
pub struct RdbcConnectionStatEntryValue {
    #[serde(rename = "ID")]
    pub id: u64,
...
/// Connection 层运行统计。
///
/// 对应 Java：`com.alibaba.druid.stat.RdbcConnectionStat`。
pub struct RdbcConnectionStat {
    ...
    connections: DashMap<u64, Arc<RdbcConnectionStatEntry>>,
}

impl RdbcConnectionStat {
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    pub fn register_entry(&self, entry: Arc<RdbcConnectionStatEntry>) {
        self.connections.insert(entry.id, entry);
    }

    /// 返回指定连接条目的共享引用。
    pub fn entry(&self, connection_id: u64) -> Option<Arc<RdbcConnectionStatEntry>> {
        self.connections
            .get(&connection_id)
    ...
    pub fn connection_entries(&self) -> Vec<RdbcConnectionStatEntryValue> {
        self.connections
            .iter()
...
impl Default for RdbcConnectionStat {
    fn default() -> Self {
        Self::new()
    }
}
