...
async fn test_pooled_connection_all_paths() {
    ...
    impl Connection for MockConn {
        async fn exec(&mut self, _: &str, _: Vec<Value>) -> Result<ExecResult, DruidError> { Ok(ExecResult { rows_affected: 1, last_insert_id: Some(42), row_count: None }) }
        async fn fetch(&mut self, _: &str, _: Vec<Value>) -> Result<Vec<Row>, DruidError> { Ok(vec![Row::new(vec![Value::Int(99)])]) }
        async fn begin(&mut self) -> Result<(), DruidError> { Ok(()) }
        async fn commit(&mut self) -> Result<(), DruidError> { Ok(()) }
        async fn rollback(&mut self) -> Result<(), DruidError> { Ok(()) }
        async fn ping(&mut self) -> Result<(), DruidError> { Ok(()) }
...
fn test_error_all_variants_display() {
    let variants = vec![
        ...
        DruidError::ConnectionLeaked { id: 1, held_for: std::time::Duration::from_secs(10) },
    ...
    // Test Error trait
    for v in &variants { assert!(std::error::Error::source(v).is_none()); }
}
...
async fn test_driver_all_methods() {
    ...
    let c2 = d.connect_with_auth("test://localhost", "u", "p").await.unwrap();
...
async fn test_after_filter_all_defaults() {
    ...
    impl AfterFilter for F {
        fn name(&self) -> &str { "f" }
        async fn after(&self, _: &ExecContext<'_>, _: &Result<ExecResult, DruidError>, _: std::time::Duration) {}
    }
    let f = F;
