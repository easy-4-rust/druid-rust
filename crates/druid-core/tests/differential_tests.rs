...
#[test]
fn test_connection_holder_initial_state() { assert_eq!(ConnectionHolder::new(1).state(), ConnectionState::Idle); }
#[test]
...
fn test_connection_holder_active_to_idle() {
    let h = ConnectionHolder::new(1);
    h.mark_active(); assert!(h.mark_idle());
...
// ── ExceptionSorter ──
#[test] fn test_pg_sorter() { assert!(PgExceptionSorter.is_exception_fatal(57001, "admin shutdown")); }
#[test] fn test_pg_sorter_non_fatal() { assert!(!PgExceptionSorter.is_exception_fatal(42601, "syntax error")); }
#[test] fn test_mysql_sorter() { assert!(MySqlExceptionSorter.is_exception_fatal(1042, "Can't get hostname")); }
#[test] fn test_null_sorter() { assert!(!NullExceptionSorter.is_exception_fatal(99999, "anything")); }
...
async fn test_begin_commit() {
    struct M { tx: bool } #[async_trait::async_trait]
...
#[test]
fn test_wrapper() { struct W; impl Wrapper for W {} assert!(!W.is_wrapper_for("anything")); }
...
async fn test_driver_connect() {
    ...
    impl Driver for MockDriver {
        ...
        async fn connect(&self, url: &str) -> Result<Box<dyn Connection>, DruidError> {
            struct C; #[async_trait::async_trait]
...
async fn test_driver_connect_with_auth_default() {
    ...
    impl Driver for MockDriver {
        ...
        async fn connect(&self, url: &str) -> Result<Box<dyn Connection>, DruidError> {
            struct C(String); #[async_trait::async_trait]
            impl Connection for C {
                async fn exec(&mut self, _: &str, _: Vec<Value>) -> Result<ExecResult, DruidError> { Ok(ExecResult::default()) }
                async fn fetch(&mut self, _: &str, _: Vec<Value>) -> Result<Vec<Row>, DruidError> { Ok(vec![]) }
                async fn begin(&mut self) -> Result<(), DruidError> { Ok(()) }
...
fn test_error_display_all_variants() {
    let cases = vec![
        ...
        (DruidError::ValidationFailed("test".into()), "validation failed: test"),
        (DruidError::ConnectionLeaked { id: 42, held_for: std::time::Duration::from_secs(10) }, "connection 42 leaked"),
        (DruidError::ConnectionDiscarded, "connection has been discarded"),
        (DruidError::DriverError("timeout".into()), "driver error: timeout"),
        (DruidError::SqlParseError("syntax".into()), "SQL parse error: syntax"),
        (DruidError::WallViolation("DROP".into()), "wall violation: DROP"),
        (DruidError::DataSourceNotFound("test".into()), "datasource not found: test"),
        (DruidError::Other("misc".into()), "misc"),
    ];
    for (err, expected) in cases { assert!(format!("{err}").contains(expected)); }
}
...
fn test_pool_config_builder_every_field() {
    let c = PoolConfig::builder()
        .name("t").url("postgres://localhost").driver_name("pg")
        .username("u").password("p").max_open(20).min_idle(5)
        .initial_size(10).acquire_timeout(std::time::Duration::from_secs(10))
        ...
        .test_on_borrow(true).test_on_return(true)
        .validation_query("SELECT 1").keep_alive(true)
        .leak_detection(true).leak_threshold(std::time::Duration::from_secs(60))
        .pool_prepared_statements(true).default_auto_commit(false)
        .break_after_acquire_failure(true).connection_error_retry_attempts(3)
        .async_close_connection(true)
        .slow_sql_threshold(std::time::Duration::from_millis(500)).build();
    assert_eq!(c.name, "t"); assert_eq!(c.max_open, 20); assert_eq!(c.min_idle, 5);
    assert!(c.test_on_borrow); assert!(c.test_on_return);
    assert!(c.validation_query.is_some()); assert!(c.keep_alive);
    assert!(c.leak_detection); assert!(c.pool_prepared_statements);
    assert_eq!(c.default_auto_commit, Some(false)); assert!(c.break_after_acquire_failure);
    assert_eq!(c.connection_error_retry_attempts, 3); assert!(c.async_close_connection);
}
...
async fn test_before_filter_all_default_hooks() {
    ...
    assert!(f.on_connection_event(&ConnectionEvent::SetNetworkTimeout(std::time::Duration::from_secs(1))).await.is_ok());
...
fn test_value_all_from_conversions() {
    let v: Value = true.into(); assert_eq!(v, Value::Bool(true));
    let v: Value = 42i64.into(); assert_eq!(v, Value::Int(42));
    let v: Value = 42i32.into(); assert_eq!(v, Value::Int(42));
    let v: Value = 3.14f64.into(); assert_eq!(v, Value::Float(3.14));
    let v: Value = String::from("hi").into(); assert_eq!(v, Value::String("hi".into()));
    let v: Value = "hi".into(); assert_eq!(v, Value::String("hi".into()));
    let v: Value = vec![1u8, 2u8].into(); assert_eq!(v, Value::Bytes(vec![1, 2]));
}
...
fn test_exec_result_all_fields() {
    let r = ExecResult::default();
    assert_eq!(r.rows_affected, 0); assert!(r.last_insert_id.is_none()); assert!(r.row_count.is_none());
    let r2 = ExecResult { rows_affected: 5, last_insert_id: Some(10), row_count: Some(100) };
    assert_eq!(r2.rows_affected, 5); assert_eq!(r2.last_insert_id, Some(10)); assert_eq!(r2.row_count, Some(100));
}
