...
use druid::dynamic::{DynamicDataSource, HighAvailableDataSource, LoadBalancer, SqlHint};
use std::sync::Arc;
...
fn random_selector_empty_candidates() {
    let ha = HighAvailableDataSource::new("empty-cand");
...
fn random_selector_single_candidate() {
    let ha = HighAvailableDataSource::new("single-cand");
...
fn random_selector_blacklist_filter() {
    let ha = HighAvailableDataSource::new("bl-filter");
    ha.insert_data_source("p1", MockPool::arc("p1"));
...
fn random_selector_all_blacklisted_fallback() {
    let ha = HighAvailableDataSource::new("all-bl");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_blacklist_snapshot() {
    let ha = HighAvailableDataSource::new("bl-snap");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_map_types() {
    let ha = HighAvailableDataSource::new("map-types");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    ha.insert_data_source("p2", MockPool::arc("p2"));
    ha.add_blacklist("p2");
...
fn random_selector_checking_interval() {
    let ha = HighAvailableDataSource::new("ci-test");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_recovery_interval() {
    let ha = HighAvailableDataSource::new("ri-test");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_validation_sleep() {
    let ha = HighAvailableDataSource::new("vs-test");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_blacklist_threshold() {
    let ha = HighAvailableDataSource::new("bt-test");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_set_target_noop() {
    let ha = HighAvailableDataSource::new("rt-noop");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_name() {
    let ha = HighAvailableDataSource::new("rname");
    let selector = RandomDataSourceSelector::new(&ha);
...
fn random_selector_init_test_on_borrow_skips() {
    let ha = HighAvailableDataSource::new("tob-init");
...
fn random_selector_init_test_on_return_skips() {
    let ha = HighAvailableDataSource::new("tor-init");
...
fn random_selector_destroy() {
    let ha = HighAvailableDataSource::new("rdestroy");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = RandomDataSourceSelector::new(&ha);
    DataSourceSelector::init(&selector);
...
fn random_selector_init_no_runtime() {
    let ha = HighAvailableDataSource::new("no-rt");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = RandomDataSourceSelector::new(&ha);
    DataSourceSelector::init(&selector);
}
...
fn random_selector_candidates_filter_busy() {
    let ha = HighAvailableDataSource::new("busy-filter");
...
fn random_selector_all_busy_returns_any() {
    let ha = HighAvailableDataSource::new("all-busy");
...
fn random_selector_blacklist_ge_map_fallback() {
    let ha = HighAvailableDataSource::new("bl-ge");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = RandomDataSourceSelector::new(&ha);
...
fn named_selector_set_and_get() {
    let ha = HighAvailableDataSource::new("named-get");
...
fn named_selector_default_name_fallback() {
    let ha = HighAvailableDataSource::new("named-default");
    ha.insert_data_source("master", MockPool::arc("master"));
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_wrong_default() {
    let ha = HighAvailableDataSource::new("named-wrong");
    ha.insert_data_source("master", MockPool::arc("master"));
    ha.insert_data_source("slave", MockPool::arc("slave"));
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_single_node() {
    let ha = HighAvailableDataSource::new("named-single");
...
fn named_selector_empty() {
    let ha = HighAvailableDataSource::new("named-empty");
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_reset() {
    let ha = HighAvailableDataSource::new("named-reset");
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_name() {
    let ha = HighAvailableDataSource::new("named-name");
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_init_noop() {
    let ha = HighAvailableDataSource::new("named-init");
    let selector = NamedDataSourceSelector::new(&ha);
    DataSourceSelector::init(&selector);
}
...
fn named_selector_destroy() {
    let ha = HighAvailableDataSource::new("named-destroy");
    let selector = NamedDataSourceSelector::new(&ha);
...
fn named_selector_ha_dropped() {
    let selector = {
        let ha = HighAvailableDataSource::new("named-dropped");
        ha.insert_data_source("p1", MockPool::arc("p1"));
...
async fn sticky_selector_reuses_pool() {
    let ha = HighAvailableDataSource::new("sticky-reuse");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    ha.insert_data_source("p2", MockPool::arc("p2"));
...
fn sticky_selector_expire_config() {
    let ha = HighAvailableDataSource::new("sticky-expire");
    let selector = StickyRandomDataSourceSelector::new(&ha);
...
fn sticky_selector_name() {
    let ha = HighAvailableDataSource::new("sticky-name");
    let selector = StickyRandomDataSourceSelector::new(&ha);
...
fn sticky_selector_set_target_noop() {
    let ha = HighAvailableDataSource::new("sticky-target");
    let selector = StickyRandomDataSourceSelector::new(&ha);
    DataSourceSelector::set_target(&selector, Some("t".to_owned()));
}
...
fn sticky_selector_init() {
    let ha = HighAvailableDataSource::new("sticky-init");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = StickyRandomDataSourceSelector::new(&ha);
    DataSourceSelector::init(&selector);
}
...
fn sticky_selector_destroy() {
    let ha = HighAvailableDataSource::new("sticky-destroy");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = StickyRandomDataSourceSelector::new(&ha);
...
fn sticky_selector_random_accessor() {
    let ha = HighAvailableDataSource::new("sticky-accessor");
    let selector = StickyRandomDataSourceSelector::new(&ha);
...
fn sticky_selector_empty() {
    let ha = HighAvailableDataSource::new("sticky-empty");
    let selector = StickyRandomDataSourceSelector::new(&ha);
    assert!(DataSourceSelector::get(&selector).is_none());
}
...
fn sticky_selector_ha_dropped() {
    let selector = {
        let ha = HighAvailableDataSource::new("sticky-dropped");
        ha.insert_data_source("p1", MockPool::arc("p1"));
...
fn sticky_selector_expired_holder() {
    let ha = HighAvailableDataSource::new("sticky-expired");
    ha.insert_data_source("p1", MockPool::arc("p1"));
    let selector = StickyRandomDataSourceSelector::new(&ha);
...
fn factory_all_valid_names() {
    let ha = HighAvailableDataSource::new("factory-all");
...
fn factory_invalid_name() {
    let ha = HighAvailableDataSource::new("factory-invalid");
