...
impl PhysicalSqlXml for TestSqlXml {
    ...
    fn character_stream(&self) -> Result<druid_core::JdbcReader, DruidError> {
        self.ensure_open()?;
        Ok(druid_core::JdbcReader::from_utf16(
            self.value
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
                .as_utf16()
...
fn row_id_url_and_sql_xml_preserve_values_streams_and_resource_lifecycle() {
    ...
    let url = druid_core::JdbcUrl::new("https://example.test/a?b=1#c");
    assert_eq!(url.external_form(), "https://example.test/a?b=1#c");
    assert_eq!(url, druid_core::JdbcUrl::from(url.external_form()));
