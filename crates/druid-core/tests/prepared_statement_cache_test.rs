...
fn prepared_statement_key_maps_all_java_constructor_overloads_and_null_error() {
    ...
    assert_eq!(m1.statement_generated_keys(), StatementGeneratedKeys::None);
    assert_eq!(m2.statement_generated_keys(), StatementGeneratedKeys::None);
    assert_eq!(m3.statement_generated_keys(), StatementGeneratedKeys::None);
    assert_eq!(
        m4.statement_generated_keys(),
        StatementGeneratedKeys::ColumnIndexes(vec![4, 5])
    );
    assert_eq!(
        m5.statement_generated_keys(),
        StatementGeneratedKeys::ColumnNames(vec!["a".to_string(), "b".to_string()])
    );
    assert_eq!(
        m6.statement_generated_keys(),
        StatementGeneratedKeys::AutoGeneratedKeys(9)
    );
    for method_type in [
        ...
        assert_eq!(
            PreparedStatementKey::new(Some("call p()".to_string()), None, method_type)
                .unwrap()
                .statement_generated_keys(),
