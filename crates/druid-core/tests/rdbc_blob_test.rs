...
impl RdbcBlobAccess for InMemoryBlobAccess {
    ...
    async fn set_binary_stream(&self, position: i64) -> Result<RdbcOutputStream, DruidError> {
        ...
        drop(state);
        Ok(RdbcOutputStream::new(BlobWriteCursor {
...
fn input_stream_clones_share_cursor_and_close_state() {
    let stream = RdbcInputStream::from_bytes([1, 2, 3, 4]);
    let clone = stream.clone();
    assert_eq!(stream, clone);
    assert_ne!(stream, RdbcInputStream::from_bytes([1, 2, 3, 4]));
    ...
    let failing = RdbcInputStream::new(FailingReader);
...
async fn blob_delegates_complete_rdbc_resource_contract_without_eager_materialization() {
    ...
    let flush_failure = RdbcOutputStream::new(FailingWriter {
