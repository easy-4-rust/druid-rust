...
impl Default for WallConfig {
    fn default() -> Self {
        Self {
            select_allow: true, select_all_column_allow: true,
            select_into_allow: true, insert_allow: true,
            update_allow: true, delete_allow: true,
            drop_table_allow: false, truncate_allow: false,
            alter_table_allow: true, create_table_allow: true,
            commit_allow: true, rollback_allow: true,
            use_allow: true, show_allow: true,
            describe_allow: true, start_transaction_allow: true,
            set_allow: true,
            update_must_have_where: true, delete_must_have_where: true,
            ...
            hint_allow: true, none_base_statement_allow: true,
            limit_zero_allow: false, comment_allow: true,
            variant_check: true, must_parameterized: false,
            metadata_allow: true, wrap_allow: true,
            deny_tables: Vec::new(), deny_functions: Vec::new(),
            deny_schemas: Vec::new(), deny_variants: Vec::new(),
            select_white_list: false, function_white_list: false,
...
impl WallConfigBuilder {
    pub fn select_allow(mut self, v: bool) -> Self { self.0.select_allow = v; self }
    ...
    pub fn multi_statement_allow(mut self, v: bool) -> Self { self.0.multi_statement_allow = v; self }
    pub fn comment_allow(mut self, v: bool) -> Self { self.0.comment_allow = v; self }
    pub fn variant_check(mut self, v: bool) -> Self { self.0.variant_check = v; self }
    pub fn limit_zero_allow(mut self, v: bool) -> Self { self.0.limit_zero_allow = v; self }
    ...
    pub fn deny_schema(mut self, s: impl Into<String>) -> Self { self.0.deny_schemas.push(s.into()); self }
    pub fn tenant_column(mut self, v: impl Into<String>) -> Self { self.0.tenant_column = v.into(); self }
