...
| Rust `druid` facade 源文件 | 1 |
| Rust 内部 core/pool/sql/stats/dynamic 源文件 | 68 |
| Rust 内置 Toasty 数据源源码 | `druid-toasty` 3 个文件 |
| 语义状态 | PARTIAL |
...
| :--- | :--- | :--- | :--- |
| Java `/core` artifact | 分散 crates | 发布边界不清 | `crates/druid` 已建立 facade |
...
| `ToastyConnectionAdapter` | Toasty `Connection` | 合法内置 Adapter；随 `druid` facade 发布 |
