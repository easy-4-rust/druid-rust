...
功能语义完全迁移，不是把现有 `druid-core` 包装后宣称完成。职责拆分只能发生在
...
| :--- | ---: | ---: | :--- |
| 生产对象/文件 | 1,644 | 当前逻辑已存在，但仍分散在过渡 crate；尚未归并到 `crates/druid/src/*` | PARTIAL |
...
| 阶段 | 范围 | 退出门禁 | 当前 |
| :--- | :--- | :--- | :--- |
| C0 | 三模块边界、对象总账、CodeGraph 基线 | 1,644 分母冻结；core 内部实现归并进 `crates/druid` | PARTIAL |
...
最终在 `druid/src/toasty` 内实现 Toasty 标准数据源，并由 `druid` 默认启用。
当前代码先落在过渡物理 crate `druid-toasty`，再由 `druid::toasty` 重导出；
这只证明能力主链存在，不代表三模块归并完成。Factory 使用 Toasty 0.9
