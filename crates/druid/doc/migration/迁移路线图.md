...
> 目标：`crates/druid` 及其内部实现 crates  
> 日期：2026-07-28

## 1. 总目标

`crates/druid` 对应 Java `/core` 模块。迁移目标是 1,644 个 Java 生产对象的
功能语义完全迁移，不是把现有 `druid-core` 包装后宣称完成。Rust 允许按职责拆成
多个内部 crate，但 `druid` 是 Java core 模块的统一发布与验收边界。

```mermaid
flowchart TB
    Java["Java /core<br/>1,644 objects"] --> Facade["crates/druid<br/>模块门面"]
    Facade --> Core["druid-core<br/>公共对象 / SPI / Filter 基础"]
    Facade --> Pool["druid-pool<br/>数据源与 native pool"]
    Facade --> Sql["druid-sql<br/>parser / AST / Wall"]
    Facade --> Stats["druid-stats<br/>SQL / datasource 统计"]
    Facade --> Dynamic["druid-dynamic<br/>HA / selector"]
    Pool --> Physical["PhysicalConnection"]
    Physical --> Toasty["druid-toasty<br/>内置标准 raw driver"]
...
| :--- | ---: | ---: | :--- |
| 生产对象/文件 | 1,644 | `druid` facade 1 文件；内部 core/pool/sql/stats/dynamic 共 68 个源码文件 | PARTIAL |
...
| 阶段 | 范围 | 退出门禁 | 当前 |
| :--- | :--- | :--- | :--- |
| C0 | crate 边界、对象总账、CodeGraph 基线 | `druid` facade 存在；1,644 分母冻结 | 已建立 |
...
新增 `druid-toasty`，并由 `druid::toasty` 重导出。Factory 使用 Toasty 0.9
