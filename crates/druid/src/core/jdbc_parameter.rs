//! 对应 Java：`com.alibaba.druid.proxy.jdbc.JdbcParameter`。
//! 来源文件：`core/src/main/java/com/alibaba/druid/proxy/jdbc/JdbcParameter.java`。

use super::{JdbcCalendar, JdbcInputStream, JdbcObject, JdbcReader};

/// Java `JdbcParameter` 自定义参数类型编号。
...
/// Java 内部 `JdbcParameter.TYPE` 常量容器。
///
/// 该内部类型没有实例状态，保留在主对象文件中。
pub struct JdbcParameterType;

impl JdbcParameterType {
    /// 二进制输入流。
...
/// `JdbcParameter#getValue()` 的 Rust 无损平台值。
///
/// Java 返回 `Object`，既可能是标量/LOB，也可能是有状态 InputStream 或
/// Reader；Rust 必须显式区分这些资源句柄，不能提前读取。
#[derive(Debug, Clone, PartialEq)]
pub enum JdbcParameterValue {
    /// 标量、LOB 或 vendor JDBC 对象。
    Object(JdbcObject),
    /// 有状态字节输入流。
    InputStream(JdbcInputStream),
    /// 有状态 UTF-16 字符 Reader。
    Reader(JdbcReader),
}

/// PreparedStatement 参数的可观察代理合同。
pub trait JdbcParameter {
