    ...
    pub fn new(time_zone_id: impl Into<String>) -> Result<Self, DruidError> {
        ...
        if time_zone_id.trim().is_empty() {
            return Err(DruidError::DriverError(
                "RDBC Calendar time zone id must not be empty".to_string(),
            ));
        }
...
pub enum RdbcCalendarArgument {
    /// 未调用 Calendar 重载。
    #[default]
    Unspecified,
    /// 调用了 Calendar 重载；内部值允许对应 Java null。
    Specified(Option<RdbcCalendar>),
}

impl RdbcCalendarArgument {
    /// 创建无 Calendar 的重载标记。
    ...
    pub fn specified(calendar: Option<RdbcCalendar>) -> Self {
