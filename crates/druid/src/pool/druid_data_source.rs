...
use crate::stats::{
    DataSourceIdentity, DataSourceMonitorable, DruidDataSourceStatManager,
    DruidDataSourceStatValue, DruidTelemetrySnapshot, PoolSnapshot, SnapshotUnavailable,
    SqlStatSnapshot, WallSnapshot,
};
use serde_json::json;
use std::sync::Arc;
...
impl DruidDataSource {
    ...
    /// 将此数据源包装为 `Arc<dyn DataSourceMonitorable>`，不注册到全局管理器。
    ///
    /// 用于需要类型化遥测 SPI 但不需要全局注册的场景。
    pub fn monitorable(self: &Arc<Self>) -> Arc<dyn DataSourceMonitorable> {
        Arc::clone(self) as Arc<dyn DataSourceMonitorable>
    }

    /// 注册到全局管理器并返回 `Arc<dyn DataSourceMonitorable>`。
    ///
    /// 等价于 `register_monitoring` + `monitorable` 的组合。
    pub fn register_monitoring_arc(self: &Arc<Self>) -> Arc<dyn DataSourceMonitorable> {
...
impl DataSourceMonitorable for DruidDataSource {
    ...
    fn identity(&self) -> DataSourceIdentity {
        let state = self.state();
        DataSourceIdentity {
            id: self.data_source_id.get().copied().unwrap_or(0),
            name: state.name,
            driver_name: Some(state.driver_name),
        }
    }
    ...
    fn try_snapshot(&self) -> Result<DruidTelemetrySnapshot, SnapshotUnavailable> {
        ...
        let state = self.state();
        let sampling_time_millis = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis() as u64;

        let pool_snapshot = PoolSnapshot {
            active_count: state.active_count as u32,
            idle_count: state.idle_count as u32,
            max_active: state.max_open as u32,
            max_idle: state.max_open as u32,
            waiting_count: state.wait_count as u32,
        };

        let sql_stats: Vec<SqlStatSnapshot> = self
            .pool
            .stats_collector()
            .sql_merger
            .all_stats()
            ...
            .map(|stat| {
                ...
                SqlStatSnapshot {
                    fingerprint: sv.sql,
                    exec_count: sv.execute_count,
                    exec_time_millis: sv.total_time_millis,
                    fetch_row_count: sv.fetch_row_count,
                    update_count: sv.update_count,
                }
            })
            .collect();

        let wall_stats = self.pool.wall_provider().stats_map();
        let wall_snapshot = if wall_stats.is_empty() {
            None
        } else {
            Some(WallSnapshot {
