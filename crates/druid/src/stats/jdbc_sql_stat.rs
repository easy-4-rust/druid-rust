...
impl JdbcSqlStat {
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#incrementRunningCount()`。
    ...
    /// 更新 SQL 管理快照的来源身份。
    ///
    /// 对应 Java：`JdbcDataSourceStat#createSqlStat` 及
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#addResultSetHoldTimeNano(long,long)`，包括其
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#incrementClobOpenCount()`。
    ...
    /// 记录成功打开一个非空 Blob。
    ///
    /// 对应 Java：`JdbcSqlStat#incrementBlobOpenCount()`。
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#addExecuteBatchCount(long)`。
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#addUpdateCount(int)`。负值和零不进入总数，
    /// 但仍进入 `<1` 档；峰值比较保留原始 JDBC 结果。
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#addFetchRowCount(long)`。
    ...
    ///
    /// 对应 Java：`JdbcSqlStat#reset()`；运行中的执行不能被重置为零。
    pub fn reset(&self) {
