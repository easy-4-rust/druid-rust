//! 对应 Java 类：`com.alibaba.druid.stat.JdbcStatManager`。

use super::{
    DruidDataSourceStatManager, JdbcConnectionStat, JdbcResultSetStat, JdbcStatContext,
    JdbcStatementStat,
};
use dashmap::DashMap;
...
pub struct JdbcStatManager {
    sql_id_seed: AtomicU64,
    connection_stat: JdbcConnectionStat,
    result_set_stat: JdbcResultSetStat,
    statement_stat: JdbcStatementStat,
    reset_count: AtomicU64,
    contexts: DashMap<ExecutionKey, JdbcStatContext>,
}

impl JdbcStatManager {
    /// 返回进程级单例。
    #[must_use]
    pub fn global() -> &'static Self {
        static INSTANCE: OnceLock<JdbcStatManager> = OnceLock::new();
        INSTANCE.get_or_init(|| Self {
            sql_id_seed: AtomicU64::new(1_000),
            connection_stat: JdbcConnectionStat::new(),
            result_set_stat: JdbcResultSetStat::new(),
            statement_stat: JdbcStatementStat::new(),
    ...
    /// 返回全局 Connection 统计。
    #[must_use]
    pub fn connection_stat(&self) -> &JdbcConnectionStat {
        &self.connection_stat
    }

    /// 返回全局 Statement 统计。
    #[must_use]
    pub fn statement_stat(&self) -> &JdbcStatementStat {
        &self.statement_stat
    }
    ...
    #[must_use]
    pub fn stat_context(&self) -> Option<JdbcStatContext> {
        self.contexts
            .get(&execution_key())
    ...
    pub fn set_stat_context(&self, context: Option<JdbcStatContext>) {
    ...
    /// 创建一个空统计上下文，不自动绑定。
    #[must_use]
    pub fn create_stat_context(&self) -> JdbcStatContext {
        JdbcStatContext::new()
    }

    /// 重置代理层全局统计与所有已注册数据源的 JDBC 统计。
    ///
