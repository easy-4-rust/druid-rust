//! 对应 Java 类：`com.alibaba.druid.stat.RdbcDataSourceStat`。
//!
//! 数据源级统计收集器。

use super::{RdbcConnectionStat, RdbcResultSetStat, RdbcSqlStat, RdbcStatementStat, SqlMerger};
...
/// 数据源统计收集器。
///
/// 对应 Druid Java 的 `RdbcDataSourceStat`，聚合池级 + SQL 级统计。
pub struct RdbcDataSourceStat {
    pub name: String,
    reset_stat_enable: AtomicBool,
    pub sql_merger: Arc<SqlMerger>,
    /// `ResultSet` 层统计；对应 Java `RdbcDataSourceStat#getResultSetStat()`。
    pub result_set_stat: Arc<RdbcResultSetStat>,
    /// `Connection` 层统计；对应 Java `getConnectionStat()`。
    pub connection_stat: Arc<RdbcConnectionStat>,
    /// `Statement` 层统计；对应 Java `getStatementStat()`。
    pub statement_stat: Arc<RdbcStatementStat>,
    // 连接级统计
...
impl RdbcDataSourceStat {
    pub fn new(name: impl Into<String>, slow_sql_threshold: Duration) -> Self {
        Self {
            ...
            result_set_stat: Arc::new(RdbcResultSetStat::new()),
            connection_stat: Arc::new(RdbcConnectionStat::new()),
            statement_stat: Arc::new(RdbcStatementStat::new()),
    ...
    /// `RdbcSqlStat#addExecuteBatchCount(long)`。
    ...
    /// 取得并重置数据源运行计数与直方图。
    pub(crate) fn runtime_snapshot_and_reset(&self) -> RdbcDataSourceRuntimeSnapshot {
        RdbcDataSourceRuntimeSnapshot {
    ...
    pub fn connection_stat(&self) -> &Arc<RdbcConnectionStat> {
        &self.connection_stat
    }

    /// 返回本数据源共享的 Statement 层统计对象。
    pub fn statement_stat(&self) -> &Arc<RdbcStatementStat> {
        &self.statement_stat
    }
    ...
    /// 返回 Java `RdbcDataSourceStat#isResetStatEnable()`。
    #[must_use]
    pub fn is_reset_stat_enable(&self) -> bool {
        self.reset_stat_enable.load(Ordering::Acquire)
    }

    /// 设置 Java `RdbcDataSourceStat#setResetStatEnable(boolean)`。
...
pub(crate) struct RdbcDataSourceRuntimeSnapshot {
...
impl Default for RdbcDataSourceStat {
    fn default() -> Self {
