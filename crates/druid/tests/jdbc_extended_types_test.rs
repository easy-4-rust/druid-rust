...
fn result_set() -> JdbcResultSet {
    JdbcResultSet::new(Arc::new(TestResultSet {
...
impl PhysicalSqlXml for TestSqlXml {
    ...
    fn character_stream(&self) -> Result<druid::core::JdbcReader, DruidError> {
        self.ensure_open()?;
        Ok(druid::core::JdbcReader::from_utf16(
            self.value
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
                .as_utf16()
    ...
    fn set_character_stream(&self) -> Result<JdbcWriter, DruidError> {
        self.ensure_open()?;
        Ok(JdbcWriter::new(TestCharacterWriter))
    }
    ...
    fn source(
        ...
        self.ensure_open()?;
        Ok(JdbcXmlSource::new(Arc::new(TestXmlSource)))
    }
    ...
    fn result(
        ...
        self.ensure_open()?;
        Ok(JdbcXmlResult::new(Arc::new(TestXmlResult)))
    }
}
...
fn row_id_url_and_sql_xml_preserve_values_streams_and_resource_lifecycle() {
    let row_id = JdbcRowId::new(vec![0, 1, 255]);
    assert_eq!(row_id.bytes(), &[0, 1, 255]);
    assert_eq!(row_id, JdbcRowId::new(vec![0, 1, 255]));

    let url = druid::core::JdbcUrl::new("https://example.test/a?b=1#c");
    assert_eq!(url.external_form(), "https://example.test/a?b=1#c");
    assert_eq!(url, druid::core::JdbcUrl::from(url.external_form()));

    let xml = JdbcSqlXml::new(Arc::new(TestSqlXml {
    ...
    let source = xml.source(&JdbcXmlRepresentationType::Dom).unwrap();
    assert_eq!(source, source.clone());
    assert!(format!("{source:?}").contains("JdbcXmlSource"));
    let result = xml.result(&JdbcXmlRepresentationType::Stream).unwrap();
    assert_eq!(result, result.clone());
    assert!(format!("{result:?}").contains("JdbcXmlResult"));
    xml.free().unwrap();
    assert!(xml.is_freed());
