...
| 本批次测试 | Java Callable oracle 101/101；Rust workspace 431/431；Toasty 内置 SQLite 3 个 + core 纵向 1 个 + SQLx/bb8/deadpool/wrapper 扩展 17 个实库用例全绿；`druid-toasty --all-features` 编译通过 | 2026-07-28 实测 |
...
flowchart TD
    ...
    Factory --> Toasty["ToastyConnectionAdapter<br/>内置标准实现"]
    Factory -. "wrapper 扩展" .-> Sqlx["SqlxConnectionAdapter"]
    Factory -. "wrapper 扩展" .-> Rbdc["RbdcConnectionAdapter"]
...
- 新增不携带 pool 的 `druid-toasty` 内置标准 raw adapter；不使用
  `toasty::Db`。`druid-sqlx` 与 `druid-rbdc` direct adapter 归入
  `druid-wrapper` 扩展。
