...
flowchart TD
    ...
    Bridge --> Bb8["Bb8ConnectionProvider"]
    Bridge --> Deadpool["DeadpoolConnectionProvider"]
    Bb8 --> Physical
    Deadpool --> Physical
...
- 修复 B-01～B-04、B-07；把 `DruidPoolConnection` 与 core `PooledConnection` 收敛为唯一公开的 `DruidPooledConnection`，归还权下沉到 `ConnectionLease`。
...
- 建立对象安全、异步、最小化的 `PhysicalConnection` SPI，以及 `PhysicalConnectionFactory`、`ConnectionProvider`、`ConnectionLease`、`LeaseReturner`。
...
| `druid_pooled_connection.rs` | `DruidPooledConnection` | 两套 pooled connection | 统一所有权、归还和代理行为 | 0.5 天 |
| `connection_lease.rs`、`lease_returner.rs` | `DruidPooledConnection.close/recycle` 所有权语义 | 当前归还逻辑分散 | 唯一租约所有者、幂等 close 与异步 Drop 归还 | 0.5 天 |
...
| `physical_connection_factory.rs` | 物理连接创建 | 当前 `ConnectionFactory` 已存在，具体 create 未实现 | 仅服务 native pool；inventory/显式 factory 双路径 | 0.25 天 |
...
| :--- | :--- | :--- | :--- | :--- |
| `druid-sqlx`（新增） | Native direct | Driver/Connection/Statement/ResultSet | 不存在 | 新增，P0 后重估 |
| `druid-rbdc` | Native direct | Driver/Connection/Statement/ResultSet | `create()` 固定未实现 | 原 2 天，按真实驱动矩阵重估 |
| `druid-sqlx-deadpool` | External bridge | 外部池租约兼容 | 被建模为 `ConnectionFactory` 且 `create()` 未实现 | 原 1.5 天，改造 Provider |
| `druid-sqlx-bb8` | External bridge | 外部池租约兼容 | 被建模为 `ConnectionFactory` 且 `create()` 未实现 | 原 1.5 天，改造 Provider |
...
| Crate | 原记录 | 当前事实 | 修订处置 |
| :--- | :---: | :--- | :--- |
| `druid-rbdc` | 7 行未覆盖 | `create()` 固定返回未实现 | 不豁免，P1 真实数据库验收 |
| `druid-sqlx-bb8` | 7 行未覆盖 | `create()` 固定返回未实现且角色错误 | 不豁免，P1 改造 Provider 并执行 bridge 验收 |
| `druid-sqlx-deadpool` | 7 行未覆盖 | `create()` 固定返回未实现且角色错误 | 不豁免，P1 改造 Provider 并执行 bridge 验收 |
