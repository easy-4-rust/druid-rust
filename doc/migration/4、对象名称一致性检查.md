...
| `DruidPooledConnection` | `DruidPooledConnection`；`PooledConnection` 仅兼容重导出 | 名称已合并，完整 Java 状态机仍待迁移 | 保持唯一 canonical 实现 |
...
| `—（close/recycle 所有权）` | `DruidPooledConnection` 内部 `FnOnce`；external 使用 `PhysicalConnectionLease` | 当前所有连接路径 exactly-once 已有契约测试 | P2 补完整 rollback/reset/discard 顺序 |
...
| `recycle()` | `DruidPooledConnection::recycle` / provider return | PARTIAL：exactly-once 已完成，完整 reset/discard 顺序待迁移 |
