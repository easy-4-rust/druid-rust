...
| Rust 基线 | commit `f307f4edf1ca76170b91d775e4d8dcdb3ff2bd29` |
| Rust 源码 | 60 个 `.rs`，51 个非模块文件 |
| Rust 公开类型 | 61 个 |
| 检查日期 | 2026-07-28 |
...
| `[Rust internal] ConnectionFactory` | `ConnectionFactory` / `connection_factory.rs` | 名称过宽，混淆 direct factory 与 external lease | PARTIAL；迁名 `PhysicalConnectionFactory` |
...
| `DruidConnectionHolder` | `ConnectionHolder` | 丢失 canonical 前缀 | `DruidConnectionHolder` / `druid_connection_holder.rs` |
| `DruidPooledConnection` | `DruidPoolConnection` + `PooledConnection` | 两套对象且归还语义分裂 | 合并为 `DruidPooledConnection` |
| `—（JDBC 平台能力）` | `Connection` trait | 容易与公开池化连接混淆，且职责过宽 | 内部 `PhysicalConnection` / `physical_connection.rs` |
| `—（物理创建能力）` | `ConnectionFactory` | 与 bb8/deadpool 外部租约职责混用 | `PhysicalConnectionFactory`，仅用于 native direct adapter |
| `—（DataSource 来源职责）` | 当前无统一对象 | native pool 与 external pool 边界不可见 | `ConnectionProvider`，实现分为 native/bridge |
| `—（close/recycle 所有权）` | callback、Drop、pool 归还逻辑分散 | 无法保证 exactly-once return | `ConnectionLease` + `LeaseReturner` |
...
| `PreparedStatementPool` | `prepared_statement_pool.rs` | MISSING |
| `PreparedStatementHolder` | `prepared_statement_holder.rs` | MISSING |
...
| `SqlxConnectionAdapter`、`RbdcConnectionAdapter` | Rust driver 生态 | 允许 | 实现 `PhysicalConnection` contract |
| `Bb8ConnectionProvider`、`DeadpoolConnectionProvider` | 外部池 bridge | 允许 | 实现 Provider contract，禁止进入 native idle queue |
...
| `DruidConnectionHolder` | `connection_holder.rs` | RENAME | canonical 类型和文件 |
| `DruidPooledConnection` | `pooled_connection.rs` | RENAME/BROKEN | 合并两套连接句柄 |
...
| `PreparedStatementHolder` | `statement_cache.rs` | MISSING | 独立文件 |
| `PreparedStatementPool` | `statement_cache.rs` | MISSING | 独立文件 |
