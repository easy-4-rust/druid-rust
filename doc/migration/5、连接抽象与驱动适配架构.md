...
| Rust 当前存在两套 pooled 句柄 | `druid/src/pool/pooled_connection.rs:12-83`、`druid/src/core/pooled_connection.rs:8-55` | 合并为唯一 `DruidPooledConnection` |
| Rust `into_core` 使用空 return callback | `druid/src/pool/pooled_connection.rs:72-75` | 删除转换逃逸路径，以 `DruidPooledConnection` 内部 `FnOnce` 固化归还权 |
| Rust `run_after_filter` 重建空上下文，`fetch` 完全绕过 Filter | `druid/src/pool/pooled_connection.rs:35-62` | Filter completion 必须由 pooled facade 统一管理 |
| Rust factory 返回 `Box<dyn Connection>`，三个 adapter 的 `create()` 均固定未实现 | `druid/src/core/connection_factory.rs:6-14`、`druid-wrapper/src/rbdc/adapter.rs:22-32`、两个 SQLx bridge 的 `adapter.rs:19-28` | direct driver 改为 `PhysicalConnectionFactory`；bb8/deadpool 改为 Provider |
| Rust dynamic route 返回 `Arc<dyn Pool>` | `druid/src/dynamic/dynamic_datasource.rs:25-37` | HA 路由目标改为 Provider/facade，不能经空 callback 丢失归还语义 |
...
| Prepared/Callable cache | `PreparedStatementPool` + `PreparedStatementHolder` + `DruidPooledPreparedStatement` + `DruidPooledCallableStatement` | Java 完整 key、access-order LRU、share/inUse、clear/schema clear、异常移除、统计；Callable 标量子集复用同一生命周期 | PARTIAL：完整 callable/setter/batch/result-set trace/Oracle 驱动行为待补 |
...
| Toasty 内置标准 | 最终 `druid/src/toasty`；当前过渡 `druid-toasty` | 直接使用 Toasty `Driver::connect` 获取 raw connection；不创建 `toasty::Db` pool；SQLite 3 个 direct + core 纵向实库契约通过；全部 driver feature 可组合编译 | SQLite 已落地；物理归并及 PostgreSQL/MySQL/Turso 真实矩阵待补，DynamoDB 不进入 SQL SPI |
| SQLx direct | 最终 `druid-wrapper/src/sqlx`；当前过渡 `druid-sqlx/src` | 真实 SQLite connect/exec/fetch/transaction/savepoint/close，并直接执行 SQLx owned `Statement` | PARTIAL：物理归并、PostgreSQL/MySQL、cancel/metadata/LOB 待补 |
| RBDC direct | 最终 `druid-wrapper/src/rbdc`；当前过渡 `druid-rbdc/src` | RBDC trait、参数/结果/事务/savepoint 与 prepared SQL token 委托契约 | PARTIAL：物理归并、真实数据库 driver 矩阵待补 |
...
token。C7 目标请求模型会在不破坏这一对象所有权关系的前提下补 timeout、fetch
size、流式 ResultSet 与 typed parameter。
...
| 最终模块内目录 | 当前过渡源码位置 | 模式 | 目标对象 | 底层对象 | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `druid-wrapper/src/sqlx` | `crates/druid-sqlx` | Native | `SqlxConnectionFactory`、`SqlxConnectionAdapter` | SQLx raw connection | 不使用 SQLx Pool |
| `druid-wrapper/src/rbdc` | `crates/druid-rbdc` | Native | `RbdcConnectionFactory`、`RbdcConnectionAdapter` | RBDC driver/connection | 加入真实 rbdc 依赖 |
| `druid-wrapper/src/sqlx_bb8` | `crates/druid-sqlx-bb8` | Bridge | `SqlxBb8Pool`、`SqlxBb8ConnectionManager`、`PhysicalConnectionLease` | bb8 pool owned lease | 已移除 native factory 角色 |
| `druid-wrapper/src/sqlx_deadpool` | `crates/druid-sqlx-deadpool` | Bridge | `SqlxDeadpoolPool`、`SqlxDeadpoolConnectionManager`、`PhysicalConnectionLease` | deadpool Object | 已移除 native factory 角色 |

四组 Adapter 是 `druid-wrapper` 的内部实现包，不是四个可独立发布的 crate。
当前物理路径仅用于追踪源码和测试；归并完成后不得保留独立版本、feature 根或产品
完成度。
...
当前仓库仍存在以下过渡物理 crate，必须按表归并后删除其独立 workspace member：
...
| `druid-sqlx`、`druid-rbdc`、`druid-sqlx-bb8`、`druid-sqlx-deadpool` | `druid-wrapper/src/{sqlx,rbdc,sqlx_bb8,sqlx_deadpool}` |
...
- 当前过渡 crate 的 Cargo 图只用于保证归并过程可编译，不是目标架构。
...
| 阶段 | 改动 | 兼容策略 | 出口 | 当前 |
| :--- | :--- | :--- | :--- | :--- |
| C0 | 冻结三个产品 crate；建立架构类型和 contract tests | 过渡 crate 在归并期间仅保留源码证据 | workspace 最终只含三个产品 crate | PARTIAL |
...
| C2-R1 | 显式 close 的 rollback/reset/testOnReturn/initSchema + disposition；脏 Drop 淘汰 | canonical holder 在 native queue/facade 间整体移动 | Java/Rust 回收事件与计数契约 | PARTIAL（主序列、schema 恢复与 schema cache 清理完成；fatal/受监管销毁待补） |
| C2-R2 | `DruidConnectionHolder` + phyTimeout/shrink/keepAlive 主分支 | `ConnectionHolder` 仅兼容 alias；显式维护 API 先于后台任务 | holder、MaxPhyTime、shrink、keepAlive 契约 | PARTIAL（holder 主干与 PS cache 完成；DestroyTask、异常补池待补） |
...
| C5 | 把 SQLx/RBDC direct 归并到 `druid-wrapper` | 真实数据库 feature gate | direct adapter contract + 无独立子 crate | PARTIAL |
...
| C8 | 删除 deprecated 旧抽象及全部过渡物理 crate | migration guide | 仅保留三个产品 crate | TODO |
