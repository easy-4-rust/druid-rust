...
> Toasty 0.9 是内置标准实现，SQLx/RBDC 是 wrapper direct 扩展。bb8/deadpool
> 作为外部连接池接入 Provider 层，禁止嵌套进入 native pool 形成双重池。
...
3. druid-rust 内置 Toasty，同时支持 SQLx、RBDC 扩展，而不把任何一个 crate 的类型泄漏到 Druid 公共 API。
...
| 边界 | 当前对象/文件 | 当前证据 | 状态 |
| :--- | :--- | :--- | :--- |
| 唯一对外连接 | `druid-core/src/druid_pooled_connection.rs` / `DruidPooledConnection` | native、`dyn Pool`、bb8、deadpool 均返回该类型；回收主链、六种 prepareStatement 与三种 prepareCall 重载接通，Drop 对脏连接安全淘汰 | 已落地；完整 callable/listener/trace/fatal 分支仍在 P2/P3 |
| canonical 生命周期容器 | `druid-core/src/druid_connection_holder.rs` / `DruidConnectionHolder` | 持有一个 `PhysicalConnection`、默认状态、时间/useCount/schema/version/discard 与惰性 `PreparedStatementPool`；native idle queue 与 pooled facade 移动同一对象 | PARTIAL：PS cache 主链完成；listener/trace、连接变量和动态密码版本失效待补 |
...
| Toasty 内置标准 | `druid-toasty` / `ToastyConnectionFactory` + `ToastyConnectionAdapter` | 直接使用 Toasty `Driver::connect` 获取 raw connection；不创建 `toasty::Db` pool；SQLite 3 个 direct + core 纵向实库契约通过；全部 driver feature 可组合编译 | SQLite 已落地；PostgreSQL/MySQL/Turso 真实矩阵待补，DynamoDB 不进入 SQL SPI |
| SQLx direct | `druid-sqlx/src/sqlx_connection_adapter.rs` | 真实 SQLite connect/exec/fetch/transaction/savepoint/close，并直接执行 SQLx owned `Statement` | PARTIAL：PostgreSQL/MySQL、cancel/metadata/LOB 待补 |
| RBDC direct | `druid-rbdc/src/rbdc_connection_adapter.rs` | RBDC trait、参数/结果/事务/savepoint 与 prepared SQL token 委托契约 | PARTIAL：真实数据库 driver 矩阵待补 |
