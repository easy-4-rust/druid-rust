...
`agents/jdbc-agent` 每进程只拥有一条 JDBC Connection。DAP1 使用有界长度帧、
协议版本、请求关联、超时、类型化标量和结构化 SQLException。当前实现覆盖
connect、query、update、prepared 参数、generic execute、transaction、ping、
close、autoCommit、readOnly 与 isolation；LOB、metadata、cancel、savepoint 与
多结果仍保持未声明能力。
...
| DuckDB | `duckdb` | 同步嵌入式 FFI driver | P2 | 使用专用 blocking worker；不得在 Tokio worker 上阻塞 |
