...
> Rust HEAD：`2c7bf3bea6f9e94f4583a85794df519c4435a425`（审计包含当前未提交迁移工作树）
...
登记为数据库驱动。MySQL、Turso 分别由 `mysql_async`、`turso` 承担。
DynamoDB capability 为非 SQL，因此不启用 Toasty DynamoDB feature，也不进入
`PhysicalConnection`。
