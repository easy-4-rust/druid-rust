...
| `—（close/recycle 所有权）` | `DruidPooledConnection` 内部三参数 `FnOnce` + `ConnectionRecycleDisposition`；native 脏 Drop 进入 `ConnectionCloseWorker`；external 使用 `PhysicalConnectionLease` | exactly-once、显式回收和 native 受监管销毁已建立 | P2 继续补 statement/schema/fatal 与 external shutdown |
...
| `WallFilter` | `sql/wall_filter.rs` | MATCH/PARTIAL | 同名独立对象和内置 alias 工厂已存在；完整 Java hook/规则仍开放 |
| `LogFilter` | `core/log_filter.rs` | MATCH/PARTIAL | canonical tracing backend、Java 默认开关、七个 properties 键、连接/Statement/ResultSet 事件和管理配置方法已落位；proxy id、完整列值与 executable SQL 格式仍受 Filter 上下文限制 |
| `LogFilterMBean` | `core/log_filter.rs` 的显式管理方法 | MERGED/PROTOCOL | Rust 无 JMX MBean；管理合同合并进线程安全 `LogFilter` API，不制造空 marker trait |
| `Slf4jLogFilter`、`Log4jFilter`、`Log4j2Filter`、`CommonsLogFilter` | `core/log_filter.rs` / tracing | MERGED/PROTOCOL | Rust 无 SLF4J/Log4j/Commons Logging；Java 类名仅保留为旧配置 alias，统一构造 tracing-backed `LogFilter` |
| `Log4jFilterMBean`、`Log4j2FilterMBean`、`CommonsLogFilterMBean` | tracing subscriber/filter 配置 + `LogFilter` 管理 API | MERGED/PROTOCOL | 三个 Java 空扩展接口没有 Rust 独立运行时对象 |
