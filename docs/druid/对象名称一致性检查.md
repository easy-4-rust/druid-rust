...
| `DruidDataSource` | `DruidDataSource` facade + `DruidPool` engine | canonical 名称和文件已建立；fatal 状态仍由同一 datasource engine 持有，没有拆出伪 Java 对象 | 继续关闭完整字段、XA 与统一证据 |
...
| `DruidStatService` | `stats/druid_stat_service.rs` | MATCH/PARTIAL：核心 JSON 路由已实现；basic 的 Java 兼容 null 键与 Rust runtime 字段已明确，统一 Drivers/web/spring 分支未闭合 |
| `DruidDataSourceStatManager` | `stats/druid_data_source_stat_manager.rs` | MATCH/PARTIAL：显式注册表已实现，JMX/OpenType 为宿主替换边界 |
...
| `FilterChainImpl` 内部 dataSource 调用游标 | `DataSourceGetConnectionFilterChain` + `DataSourceReleaseConnectionFilterChain` + `DataSourceConnectionProvider`（同 `filter_chain_impl.rs`） | RUST_SUPPORT/PROTOCOL：支撑 Java chain position 与 core→pool 依赖倒置，不计新增 Java 对象 |
...
| `StatFilter` | `stats/stat_filter.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | query/update/generic execute、普通与 PreparedStatement batch、running/concurrent、活动事务、ResultSet fetch/hold、读取量与数据源/SQL 双层 LOB open 生产链已接；JVM stack trace、完整字段及统一差分待补 |
...
| `LogFilter` | `core/log_filter.rs` | MATCH/PARTIAL | canonical tracing backend、Java 默认开关、七个 properties 键、连接/Statement/ResultSet 事件和管理配置方法已落位；SQL/batch、Statement 创建/关闭、connection 事务/关闭及 ResultSet 已携带 proxy ID，完整列值与 executable SQL 格式仍开放 |
...
| `DruidStatService` | `stats/druid_stat_service.rs` | MATCH/PARTIAL | service/protocol 对象已存在，web/spring 分支待补 |
| `DruidDataSourceStatManager` | `stats/druid_data_source_stat_manager.rs` | MATCH/PARTIAL | canonical 显式 Arc registry 已存在 |
| `DruidStatManagerFacade` | `stats/druid_stat_manager_facade.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | canonical facade、reset、数据源/SQL/Wall/连接、SQL 列表过滤、Wall 递归聚合及真实 StartTime/Rust runtime 管理边界已存在；Drivers 与 web/spring 聚合待补 |
| `DataSourceMonitorable` | `stats/data_source_monitorable.rs` | MATCH/PARTIAL | canonical trait 与 DruidDataSource 实现已存在 |
