...
| `DruidPooledResultSet` | 同名独立文件 | PASS | PARTIAL：完整 getter/update/metadata/Wrapper 基础上，next/close/warning、18 个标量、16 个 decimal/temporal、六个 plain/map/typed object、26 个 stream/resource getter、26 个 navigation/property 调用及两条 NString 重载同步 Filter around-chain，精确重载、scale、Calendar 三态、nullable Map/target type、共享资源句柄、导航参数、Unicode、顺序、短路、改写及错误分类已验证；`is_closed_with_connection` 与内部 `is_closed` 职责分离；StatFilter open/close/fetch/hold、显式与 Statement 级联关闭已落；custom typed object 单次物理读取、默认 SPI 精确能力错误及 94 个 ResultSet 精确委托已验证；Statement/PreparedStatement canonical query 保留同一物理 ResultSet，SQLx SQLite 普通零行/prepared 与 RBDC SPI 的 driver label 已测；Toasty eager/raw SQL alias 缺失、RBDC 空结果 descriptor、嵌套 ResultSet/Clob object 自动代理、metadata/getStatement/update Filter、SQL read length、custom conversion、streaming 与多数据库待补 |
...
| `updateRef/updateBlob/updateClob/updateArray/updateRowId/updateNClob/updateSQLXML(int/String,对象)` | `update_*` + `update_*_by_label` | 14 个对象型重载及 Java null 身份已闭合 |
| `updateBlob(InputStream)`、`updateClob/updateNClob(Reader)` | `update_*_stream/reader*` | index/label、无长度/long 长度共 12 重载已闭合，流不预读 |
