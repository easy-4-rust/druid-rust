...
| `RdbcInputStream`/`RdbcOutputStream` | `java.io.InputStream/OutputStream` | 合法平台 Adapter；共享游标/关闭状态，不得用 Vec 代替资源句柄 |
...
| `HighAvailableDataSource` | 同名 canonical facade；`DynamicDataSource` 仅 Rust 读写分离扩展 | 已纠正，不再由 ArcSwap facade 冒充 Java HA；PARTIAL/IMPLEMENTED_UNVERIFIED | File/ZooKeeper node、update、selector task 与 close 已接；真实 ensemble 和最终差分仍缺 |
...
| `SQLUtils` | `sql/sql_utils.rs` / `SqlUtils` | MATCH/PARTIAL | parse/单语句/format/parameterize 和显式方言选择已接；Java 全 overload、AST/Visitor/输出细节仍缺 |
| `SQLParserFeature` | `sql/sql_parser_feature.rs` / `SqlParserFeature` | MATCH/IMPLEMENTED_UNVERIFIED | 29 项 ordinal/mask/name 及 isEnabled/config/of/null varargs 已接；统一 Java 差分待补 |
| `DialectFeature` | `sql/dialect_feature.rs` / `DialectFeature` | MATCH/IMPLEMENTED_UNVERIFIED | inner Feature 映射为联合枚举，LexerFeature/ParserFeature 保留全部常量、alias、mask 与四构造器时序 |
| `SQLType` | `sql/sql_type.rs` / `SqlType` | MATCH/IMPLEMENTED_UNVERIFIED | 128 项原分类名称和 ordinal 已保留；parser 分类生产链及统一 Java 差分待补 |
| `LayoutCharacters` | `sql/layout_characters.rs` / `LayoutCharacters` | MATCH/IMPLEMENTED_UNVERIFIED | TabInc/TAB/LF/FF/CR/EOI 六常量已迁移 |
| `CharTypes` | `sql/char_types.rs` / `CharTypes` | MATCH/IMPLEMENTED_UNVERIFIED | hex/digit/identifier/whitespace/cache/UTF-16 trim 已迁移；保留 Java code-unit 256 历史越界点，统一差分待补 |
| `Token` | `sql/token.rs` / `Token` | MATCH/IMPLEMENTED_UNVERIFIED | 335 项声明顺序、values/ordinal/valueOf、原枚举名及 23 项 nullable public name 已迁移 |
| `Keywords` | `sql/keywords.rs` / `Keywords` | MATCH/IMPLEMENTED_UNVERIFIED | DEFAULT 104 项、SQLite/DM 独立对象、containsValue/getKeyword/getKeywords、UTF-16 ASCII-lower FNV 与 signed-long 二分已迁移 |
| `Lexer` | `sql/lexer.rs` / `Lexer` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | canonical 独立文件已建立；Java inner `SavePoint/CommentHandler` 按规范同文件映射为 `LexerSavePoint/CommentHandler`；UTF-16 通用状态机已落，方言专用入口和全 feature 分支待补 |
| `ParserException` | `sql/parser_exception.rs` / `ParserException` | MATCH/IMPLEMENTED_UNVERIFIED | nullable message/cause、line/column 与五构造路径已迁移 |
| `EOFParserException` | `sql/eof_parser_exception.rs` / `EofParserException` | Rust acronym 惯例 MATCH/IMPLEMENTED_UNVERIFIED | 继承映射为 Deref，固定 `EOF` message |
| `NotAllowCommentException` | `sql/not_allow_comment_exception.rs` / `NotAllowCommentException` | MATCH/IMPLEMENTED_UNVERIFIED | 默认/消息/消息加 cause 三构造器已迁移 |
| deprecated `SQLParseException` | `sql/sql_parse_exception.rs` / `SqlParseException` | Rust acronym 惯例 MATCH/IMPLEMENTED_UNVERIFIED | Java 空类与隐式无参构造器映射为 deprecated unit error |
| `SymbolTable` | `sql/symbol_table.rs` / `SymbolTable` | MATCH/IMPLEMENTED_UNVERIFIED | inner Entry 与主对象同文件；三 addSymbol/findSymbol/global 的缓存、碰撞和对象身份已迁移 |
| `SQLInsertValueHandler` | `sql/sql_insert_value_handler.rs` / `SqlInsertValueHandler` | Rust acronym 惯例 MATCH/IMPLEMENTED_UNVERIFIED | 14 回调与 Java overload/type/row identity 已保留；生产 parser 待接 |
...
| `EncodingConvertFilter` | `core/filter/encoding/encoding_convert_filter.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | alias/属性配置、SQL/prepare/addBatch、Clob 和 ResultSet 转码已落位；Prepared/Callable 参数描述符待补 |
...
| `WallConfig` | `wall_config.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | 字段与默认值已纠正，部分行为待接线 |
| `WallVisitor` | `wall_visitor.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | 独立 trait，保留 Java 可观察状态合同 |
| `WallVisitorBase` | `wall_visitor_base.rs` | MATCH/PARTIAL/IMPLEMENTED_UNVERIFIED | 独立 base，承载 deny/readonly/variant 公共遍历 |
| `WallCheckResult` | `sql/wall_check_result.rs` | MATCH/PARTIAL | 独立结果对象已存在；完整 Java AST/统计字段待补 |
| `WallUtils` | `sql/wall_utils.rs` | MATCH/IMPLEMENTED_UNVERIFIED | 五种 Java 公开方言的默认/显式配置入口名称均可追溯 |
...
| `TableStat` | `stats/table_stat.rs` / `TableStat` | MATCH/IMPLEMENTED_UNVERIFIED | 主对象及 Java inner Name/Relationship/Condition/Column/Mode 在同文件迁移；统一 Java 差分待补 |
