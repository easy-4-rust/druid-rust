...
| `DruidPooledResultSet` | 同名独立文件 | PASS | PARTIAL：完整 getter/update/metadata/Wrapper 基础上，next/close/warning、18 个标量、16 个 decimal/temporal 及六个 plain/map/typed object getter 同步 Filter around-chain，精确重载、scale、Calendar 三态、nullable Map/target type、顺序、短路、改写及错误分类已验证；StatFilter open/close/fetch/hold、显式与 Statement 级联关闭已落；custom typed object 单次物理读取、默认 SPI 精确能力错误及 40 个强类型物理重载委托已验证；Statement/PreparedStatement canonical query 保留同一物理 ResultSet，SQLx SQLite 普通零行/prepared 与 RBDC SPI 的 driver label 已测；Toasty eager/raw SQL alias 缺失、RBDC 空结果 descriptor、嵌套 ResultSet/Clob object 自动代理、stream/resource/navigation/property/metadata/update Filter、SQL read length、custom conversion、streaming 与多数据库待补 |
