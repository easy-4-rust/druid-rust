...
能输出真实值，当前仍为空并保持 PARTIAL。该切片是管理协议兼容与 Rust 原生
边界的组合，不创建任何 Java 运行时对象；统一 Axum host/JSON golden 前只记
`V0_STATIC + V1_RUST_LOCAL`。
...
- 检查顺序固定为 closed 后 disabled，分别返回 `PoolClosed` 与
  `DataSourceDisabled`，不再由 facade 提前检查 enable 而掩盖 closed；
...
- SQL 按 Java UTF-16 `String#length/substring(0, 1024)` 截断并保存为
  `JavaString`，包括边界落在 surrogate 中间时的无损状态；连接级错误仍保存
  null SQL；
...
Java timeout message 中的 createStart/creatingCount/createErrorCount/running SQL
拼接、notFullTimeoutRetry、同步/异步 initExceptionThrow 以及统一并发/取消/live
diff 仍开放。本切片仍只执行 V0 静态门禁，状态为
`PARTIAL/IMPLEMENTED_UNVERIFIED`。
