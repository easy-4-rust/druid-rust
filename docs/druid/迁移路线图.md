...
- 单字符 cache 只为 Java 0..255 的 identifier code unit 产生 `JavaString`；
...
- 同时提供无损 `JavaString` 查询，未用 Unicode lowercase 污染 Java规则；
...
- 所有 SQL/函数字符串使用 `JavaString` 保留 UTF-16；
...
- 原始输入使用 `JavaString`，`pos/mark/bufPos/ch` 全部按 UTF-16 code unit；
