...
| Rust 迁移批次基线提交 | `38620666a570bd5dbbf15a0dd86a3caea9e92c32` | 本批次修改前 `git rev-parse HEAD` |
| Rust 当前规模 | CodeGraph 索引 117 个 `.rs`；布局审计扫描 85 个生产对象文件，0 error/0 warning | 2026-07-28 CodeGraph/源码扫描 |
| 本批次测试 | Java Callable oracle 101/101；三 crate Rust workspace 433/433；Toasty 内置 SQLite 3 个 + core 纵向 1 个 + SQLx/bb8/deadpool/wrapper 扩展 17 个实库用例全绿；`druid --all-features` 编译通过 | 2026-07-29 实测 |
...
| `druid_pooled_connection.rs` | `DruidPooledConnection` | 唯一实现并独占 `DruidConnectionHolder`；显式 close 已完成 rollback/reset/testOnReturn/initSchema/处置分类，六种 prepareStatement 重载和缓存主链已接通 | 继续迁移 callable、statement 属性/listener/trace、fatal 与受监管销毁 | 0.5 天 |
