...
| JDBC Agent direct | `druid-wrapper/src/jdbc_agent` + `agents/jdbc-agent` | JSON-RPC 2.0 NDJSON、ready/handshake、共享进程、多 session 隔离、空闲 TTL、JDBC 类型/错误转换、query/update/prepared/transaction/ping/close | PREVIEW：H2 Java 与 Rust 双 session 跨语言契约；厂商实库、LOB/metadata/cancel/savepoint/multi-result 待补 |
...
flowchart TB
    ...
    subgraph native ["Native Pool 模式"]
        ...
        AgentFactory --> AgentAdapter["JdbcAgentConnection<br/>shared JSON-RPC session"]
    end
