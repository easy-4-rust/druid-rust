...
`RdbcInputStream/RdbcOutputStream` 保留共享游标和关闭状态；5 个 Callable Blob
...
`JdbcNClob + PhysicalNClob`、`RdbcReader/RdbcWriter` 表达平台资源，
`RdbcString` 以 UTF-16 code unit 无损保存 Java String。19 个 Callable 入口
...
- 迁移 ConfigFilter、LogFilter、EncodingConvertFilter 的业务结果；canonical
  对象和配置名称必须保留，宿主边界可分别接 secret manager、tracing layer、
  driver codec，但不得用替换组件抹掉 Druid 原语义。
...
| `LogFilter` / Java 日志框架边界 / MBean 合同 | tracing-backed `LogFilter` | PARTIAL：仅保留 Druid Filter 配置短名 `log`，运行时后端统一为 `tracing`；Java backend 类名标记 NOT_APPLICABLE，MBean 合同合并为管理 API；proxy id、结果列、动态 level 与 executable SQL 精确格式待补 |
...
| `ConfigFilter` / `ConfigTools` | canonical 对象 + reqwest/rustls + RustCrypto RSA；secret manager 仅作可选宿主扩展 | PARTIAL/IMPLEMENTED_UNVERIFIED：优先级、文件/classpath/HTTP(S)、properties/XML、旧密文与工厂前置解析已落位；统一差分/真实数据源验证待做 |
...
`JdbcBlob`、`JdbcClob`、`JdbcNClob`、`RdbcReader`、`RdbcWriter`、
`RdbcString` 行/函数/Regions 100%，`RdbcInputStream/RdbcOutputStream` 行覆盖
