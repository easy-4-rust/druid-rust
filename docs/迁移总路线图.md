...
flowchart TB
    ...
    Wrapper --> Bb8["druid_wrapper::sqlx_bb8<br/>SqlxBb8Pool"]
    Wrapper --> Deadpool["druid_wrapper::sqlx_deadpool<br/>SqlxDeadpoolPool"]
    Bridge --> Bb8
    Bridge --> Deadpool
    Bb8 --> Physical
    Deadpool --> Physical
...
| `druid-wrapper` | `src/sqlx`、`src/rbdc`、`src/sqlx_bb8`、`src/sqlx_deadpool` | 对各种数据库操作实现、驱动和外部连接池的封装 |
...
| B-06 | PARTIAL | `testOnReturn`、默认连接属性、rollback/reset、最大使用次数、`phyTimeout`、显式 `shrink` 与 keepAlive 检查已接线；后台 DestroyTask、keepAlive 失败后的 minIdle 回填、`initialSize`、`testWhileIdle` 等仍开放 |
...
| `druid_pooled_connection.rs` | `DruidPooledConnection` | 唯一实现并独占 `DruidConnectionHolder`；显式 close、prepare/cache/Wrapper 及 23 条物理连接错误路径的 sorter→fatal→holder/物理 Adapter discard 已接通 | 继续迁移 Statement 全操作、listener/trace/统计与受监管销毁 | 0.5 天 |
| `physical_connection_lease.rs` + `ConnectionRecycleDisposition` + `DruidPooledConnection` 内部 returner | `DruidPooledConnection.close/recycle` 所有权语义 | exactly-once、显式异步回收及脏 Drop 淘汰已验证 | 补受监管销毁、shutdown 排空和 external 真实驱动矩阵 | 0.5 天 |
...
| `druid-wrapper/src/sqlx_deadpool` | `druid-sqlx-deadpool` | External bridge | 外部池租约兼容 | `SqlxDeadpoolPool` 直接实现 core `Pool`，Object 归还 deadpool | 补 broken lease/状态映射 |
| `druid-wrapper/src/sqlx_bb8` | `druid-sqlx-bb8` | External bridge | 外部池租约兼容 | `SqlxBb8Pool` 直接实现 core `Pool`，owned lease 归还 bb8 | 补 broken lease/状态映射 |
...
| async Drop 无法执行异步 reset | 显式 `close().await` 执行完整回收；Drop 仅复用干净连接，脏连接直接淘汰；后续补受监管销毁与 shutdown 排空 |
