...
> **当前口径说明（2026-07-28）**：本文保留 2026-07-27 的设计阶段基线，
> 其中“占位骨架”、旧 `ConnectionFactory`/`PooledConnection` 名称和
> bb8/deadpool 作为 factory 的描述不代表当前实现。连接架构与迁移状态以
> [连接抽象与驱动适配架构](./doc/migration/5、连接抽象与驱动适配架构.md)
> 以及迁移目录中的对象/语义总账为准。项目目标是 Druid 功能语义完整迁移，
> 不是仅借鉴架构。
